...
const Department = () => {
  ...
  const columns =[
    ...
    { field: 'action', headerName: 'Action', width:140 ,
         renderCell: (params) => (
          ...
          <IconButton onClick={() => SweetAlert({ helpfn :alertActionFn('oktest')}) }>
  ...
  let alertActionFn = (props) =>{
    console.log(props, '==== action function called ====')
  }

  // const deleteApi = async (id) =>{
  //   console.log('========= called delete =============')
  //   let res = await categoryDeleteApi(id,cookies.token);
  //   if(res.status===true){
  //     getApi();
  //     setactionType('list');
  //   }
  ...
  return (
    ...
    <Row>
      ...
      <Col xs={12} className='overflow-hidden my-3'>
        <CustomTable rows={categoryData} columns={columns}  getRowId={getRowId}/>
      </Col>
    </Row>
